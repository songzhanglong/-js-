...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        //delete
        UIButton *button=[UIButton buttonWithType:UIButtonTypeCustom];
        [button setFrame:CGRectMake(frame.size.width - 100 - 6, (frame.size.height - 33) / 2, 100, 33)];
        [button setTag:3];
...
- (void)pressButton:(id)button
{
    ...
    if(_lx==1){
        if([button tag]==3){
            ...
            UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:@"提醒" message:@"是否删除消息？" delegate:self cancelButtonTitle:@"否" otherButtonTitles:@"是", nil];
            alertView.tag=index;
            [alertView show];
            return;
        }
