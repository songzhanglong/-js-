...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        if (manager.userInfo) {
            nSelecetedIdx = [manager.grades indexOfObject:manager.userInfo];
        }
        else
        {
            nSelecetedIdx = 0;
...
{
    return [DJTGlobalManager shareInstance].grades.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    DJTGlobalManager *manager = [DJTGlobalManager shareInstance];
    DJTUser *user = manager.grades[indexPath.row];
    ...
    [headImg setImageWithURL:[NSURL URLWithString:face] placeholderImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"s21@2x" ofType:@"png"]]];
    [label setText:[user.grade_name ?: @"" stringByAppendingString:user.classname]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.userInteractionEnabled = NO;
    NSArray *children = [DJTGlobalManager shareInstance].grades;
    DJTUser *user = [DJTGlobalManager shareInstance].userInfo;
