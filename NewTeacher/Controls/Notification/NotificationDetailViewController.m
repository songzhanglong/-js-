...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 2) {
        ...
        }else if (![[DJTGlobalManager shareInstance].userInfo.userid isEqualToString:item.author_id] && [item.is_teacher isEqualToString:@"1"]){
            if (!_inputBar.textField.isFirstResponder) {
                [_inputBar.textField becomeFirstResponder];
            }
            UIColor *color = [UIColor blackColor];
        ...
        else
        {
            ...
            MileageAllEditView *editView = [[MileageAllEditView alloc] initWithFrame:[UIScreen mainScreen].bounds Titles:@[@"回复", @"删除"]];
            editView.delegate = self;
            [editView showInView:self.view.window];
        }
    }
