        ...
        if ([_themeBatch.photos count] == 1) {
            ...
            if (item.type.integerValue != 0) {
                ...
                if (mp4) {
                    ...
                    [imgView setImageWithURL:[NSURL URLWithString:[tmpStr stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
                }
            }
            else{
...
- (void)diggestFinish:(BOOL)suc Data:(id)result
{
    ...
    if (suc) {
        ...
        if (_themeBatch.have_digg.integerValue == 0) {
            ...
            item.mid = @"0";
            item.name = userInfo.uname;
            item.author_id = userInfo.userid;
            item.is_teacher = @"1";
            ...
            for (BatchDetailDiggItem *item in _batchDetail.diggList) {
                if ([item.author_id isEqualToString:userInfo.userid]) {
...
- (void)requestFinish:(BOOL)success Data:(id)result
{
    ...
    if (success) {
        ...
        //self.titleLable.text = detail.name;
        UIButton *button = (UIButton *)[_tableView.tableHeaderView viewWithTag:1];
        button.selected = (_themeBatch.have_digg.integerValue != 0);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        [(ThemeDetailViewCell *)cell resetHead:_themeBatch.face Name:_themeBatch.name Time:timeStr Con:_themeBatch.digst Hei:_themeBatch.contentHei];
    }
    else if (indexPath.section == 1) {
