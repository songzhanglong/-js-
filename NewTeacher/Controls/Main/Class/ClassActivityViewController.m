...
- (void)backToPreControl:(id)sender
{
    if ([_selectAlbums count] > 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"是否将图片导入模版？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
        [alert show];
        return;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    select.nMaxCount = _nMaxCount - [_selectAlbums count];
    select.otherArr = _selectAlbums;
    select.photoItem = activity;
    //select.classActivity = activity;
