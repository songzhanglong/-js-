...
@interface AddActivityViewController ()<DeleteImageViewDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,CTAssetsPickerControllerDelegate,SendToMotherDelegate,UIScrollViewDelegate,UITextViewDelegate,MWPhotoBrowserDelegate,BabyMileageToClassDelegate,MileageAllEditViewDelegate>
...
- (void)uploadImgsOrVideo:(NSString *)url Arr:(NSArray *)array
{
    ...
    if (_peoples.count > 0) {
        ...
        for (DJTStudent *student in _peoples) {
            NSDictionary *dic = @{@"member_id":student.student_id,@"is_teacher":@"0",@"member_name":student.uname};
            [ids addObject:dic];
        }
...
- (void)addImageView:(id)sender
{
    ...
    else
    {
        if ([self.dataSource count] >= 20) {
            [self.view makeToast:@"图片一次最多只能添加20张" duration:1.0 position:@"center"];
            return;
        }
...
- (void)selectEditIndex:(NSInteger)index
{
    switch (index) {
        case 0:
        {
            ...
            picker.maximumNumberOfSelection = 20 - [self.dataSource count];
...
- (void)setImagePathSubViews
{
    ...
    for (NSInteger i = 0; i < [self.dataSource count]; i++) {
        ...
        deleteImg.tag = i + 1;
        deleteImg.delegate = self;
        if ([subObj isKindOfClass:[ALAsset class]]) {
            ALAsset *asset = (ALAsset *)subObj;
            [deleteImg setImage:[UIImage imageWithCGImage:asset.thumbnail]];
        }
        else
        {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil)
    {
        ...
        if (indexPath.section == 0)
        {
            ...
            [sendButton setImage:CREATE_IMG(@"bb2_2") forState:UIControlStateNormal];
...
- (void)deleteImageView:(DeleteImageView *)imageView
{
    if (_isVideo) {
        if (_videoPath) {
            ...
            [_addBtn setFrame:CGRectMake(10, butRec.origin.y, butRec.size.width, butRec.size.height)];
            _addBtn.hidden = NO;
        }
    }
    else
...
- (void)assetsPickerController:(CTAssetsPickerController *)picker didFinishPickingAssets:(NSArray *)assets
{
    ...
    for (int i=0; i<[array count]; i++) {
        ...
        for(int j=0;j<count;j++){
            id object = [self.dataSource objectAtIndex:j];
            if ([object isKindOfClass:[ALAsset class]]) {
                ALAsset *_dataAsset=object;
