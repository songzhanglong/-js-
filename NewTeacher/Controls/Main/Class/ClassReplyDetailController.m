...
- (UIView *)createTableHeaderView
{
    ...
    if (_circleModel.imagesRect.size.height > 0) {
        ...
        [resumeImageView.morePicture setHidden:YES];
...
- (void)replyContent:(NSString *)content
{
    ...
    if (_indexPath) {
        ...
        [param setObject:item.send_name forKey:@"reply_name"];
    }
...
{
    DJTGlobalManager *manager = [DJTGlobalManager shareInstance];
    NSString *str = [NSString stringWithFormat:@"http://wap.goonbaby.com/dynamic_share/t%@_b%@_f1.htm",_circleModel.tid,manager.userInfo.userid];
    switch (index) {
        case 0:
        case 1:
        case 2:
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([item.send_name isEqualToString:[DJTGlobalManager shareInstance].userInfo.uname]) {
        //是自己发的消息
        _indexPath = nil;
        UIColor *color = [UIColor whiteColor];
        _inputBar.textField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:@"说点什么..." attributes:@{NSForegroundColorAttributeName: color}];
    ...
    {
        UIColor *color = [UIColor whiteColor];
        _inputBar.textField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"回复%@:",item.send_name] attributes:@{NSForegroundColorAttributeName: color}];
    }
}
