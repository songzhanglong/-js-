...
#pragma mark - 控制器
@interface MakeGrowController ()<CanCancelImageViewDelegate,MakeViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,EditGrowBarDelegate,SelectPhotosViewControllerDelegate,EditTextViewDelegate,MWPhotoBrowserDelegate,AddTextViewControllerDelegate>
...
- (void)viewDidLoad
{
    ...
    [lefBut setFrame:CGRectMake(0, 0, 40, 25)];
    ...
    saveBtn.frame = CGRectMake(0, 0, 50.0, 22.0);
...
- (void)toolBarCreate
{
    ...
    for (int i = 0 ; i < 2; i++) {
        ...
        if (i == 1)
        {
            [but setFrame:CGRectMake(winSize.width - 100, 11.5, 90, 21)];
            [but setBackgroundColor:rgba(25, 161, 86, 1)];
            [but setTitle:@"同步到其他学生" forState:UIControlStateNormal];
            [but.titleLabel setFont:[UIFont systemFontOfSize:12]];
        ...
        {
            //CGFloat diff = (winSize.width - 50 * 3 - margin * 2) / 2;
            [but setFrame:CGRectMake(0, 0, 50, 30)];
            [but setCenter:CGPointMake(_toolbar.bounds.size.width / 2, _toolbar.bounds.size.height / 2)];
            ...
            [but setTitleColor:CreateColor(244, 174, 97) forState:UIControlStateHighlighted];
        }
    ...
    //line
    CGFloat diff = (winSize.width - 50 * 3 - margin * 2) / 2;
    _lineView = [[UIView alloc] initWithFrame:CGRectMake(margin + 50 + diff, 37, 50, 1)];
...
- (void)disappearDecorateView:(BOOL)shouldCommit End:(void (^)(void))completion
{
    if (_decorateView && [_decorateView isDescendantOfView:self.view]) {
        UIButton *midBut = (UIButton *)[_toolbar viewWithTag:1];
        [UIView animateWithDuration:0.35 animations:^(void) {
...
- (void)downHistoryImgs:(NSInteger)index
{
    ...
    if (!_growAlbum.src_video_list || ![_growAlbum.src_video_list isKindOfClass:[NSArray class]]) {
        return;
    }
...
{
    NSDictionary *dic = @{@"page":@"1",@"page_num":@"100",@"teacher_id":[DJTGlobalManager shareInstance].userInfo.userid};
...
- (void)commitLastInfo
{
    ...
    NSMutableDictionary *dic = [NSMutableDictionary dictionaryWithDictionary:@{@"grow_id":_student.grow_id,@"templist_id":_student.templist_id,@"term_id":_student.term_id,@"userid":user.userid,@"student_id":_student.student_id,@"school_id":user.schoolid,@"class_id":user.classid,@"template_id":_growAlbum.template_id}];
...
- (void)commitInfoFinish:(BOOL)suc Data:(id)data
{
    ...
    if (suc) {
        ...
        //返回数据
        if (_isSyncsize) {
            SynchronizeViewController *synch = [[SynchronizeViewController alloc] init];
            synch.student = _student;
            synch.growAlbum = _growAlbum;
            synch.dataSource = _dataSource;
