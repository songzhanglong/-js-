...
@interface EditFamilyViewController () <FamilyEditListCellDelegate,CreateFamilyViewDelegate,UIAlertViewDelegate>
{
...
- (void)viewDidLoad {
    ...
    NSDictionary *dic = @{@"school_id":manager.userInfo.schoolid,@"class_id":manager.userInfo.classid,@"grade_id":manager.userInfo.grade_id,@"student_id":_listItem.student_id};
    [self createTableViewAndRequestAction:@"form:form_score" Param:dic Header:YES Foot:NO];
    ...
    if (_isRefreshData) {
        for (id controller in self.navigationController.viewControllers) {
            if ([controller isKindOfClass:[FamilyViewController class]]) {
                [controller setIsRefreshStudentData:_isRefreshData];
            }
        }
        [self beginRefresh];
        _isRefreshData = NO;
...
- (void)addAction:(id)sender
{
    ...
    [createView createTableView:_listItem];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    EditFamilyModel *model = [self.dataSource objectAtIndex:indexPath.section];
    _listItem.form_id = model.form_id;
    _listItem.form_date = model.form_date;
    ...
    detailController.listItem = _listItem;
...
- (void)deleteSection:(UITableViewCell *)cell
{
    ...
    if ([manager.userInfo.userid isEqualToString:model.create_user]) {
        ...
        [self.view makeToast:@"教师能删除自己的联系表" duration:1.0 position:@"center"];
    }
}
...
- (void)sendDeleteRequest
{
    ...
    NSDictionary *dic = @{@"school_id":manager.userInfo.schoolid,@"class_id":manager.userInfo.classid,@"grade_id":manager.userInfo.grade_id,@"student_id":model.student_id,@"score_id":model.id};
