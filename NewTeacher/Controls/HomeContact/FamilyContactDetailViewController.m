...
- (void)viewDidLoad {
    ...
    NSDictionary *dic = @{@"school_id":manager.userInfo.schoolid,@"class_id":manager.userInfo.classid,@"grade_id":manager.userInfo.grade_id,@"student_id":_listItem.student_id,@"form_date":_listItem.form_date,@"form_id":_listItem.form_id};
...
- (void)requestFinish:(BOOL)success Data:(id)result
{
    ...
    if (success) {
        ...
        _comment = [ret_data valueForKey:@"comment"];
...
- (void)createTableHeaderView:(NSDictionary *)dictory
{
    ...
    CGFloat _contHei = [self caculateContentToHeight:[dictory valueForKey:@"comment"]];
    ...
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, (_fromType == 2) ? 5 : 10, SCREEN_WIDTH - 20, 30)];
    [titleLabel setBackgroundColor:(_fromType == 2) ? [UIColor clearColor] :CreateColor(235, 235, 240)];
    [titleLabel.layer setCornerRadius:2];
    [titleLabel setText:[dictory valueForKey:@"title"] ?: @""];
    [titleLabel setTextColor:(_fromType == 2) ? CreateColor(43, 185, 52) : [UIColor grayColor]];
    [titleLabel setFont:(_fromType == 2) ? [UIFont boldSystemFontOfSize:14] : [UIFont systemFontOfSize:14]];
    ...
    {
        [_sliceColors addObject:[arr objectAtIndex:i%5]];
    }
    ...
    [contLabel setText:[dictory valueForKey:@"comment"] ?: @""];
...
- (UIView *)setFooterView
{
    ...
    [editView.placeholderLab setText:[NSString stringWithFormat:@"回复%@的%@：",_indexItem.create_user_name,([_indexItem.relation length] > 0 ? _indexItem.relation : @"老师")]];
...
- (void)replyTeacher:(EditTextView *)editTextView
{
    ...
    if ([_indexItem.reply_id length] > 0) {
        relate_user_name = [NSString stringWithFormat:@"%@的%@",_indexItem.create_user_name,_indexItem.relation];
        reply_id = _indexItem.reply_id;
    }
    NSString *url = [URLFACE stringByAppendingString:@"form:form_reply_save"];
    NSDictionary *dic = @{@"school_id":manager.userInfo.schoolid,@"class_id":manager.userInfo.classid,@"form_id":_listItem.form_id,@"student_id":_listItem.student_id,@"teacher_id":manager.userInfo.userid,@"content":[editTextView.textView text],@"score_id":_score_id,@"relate_id":reply_id,@"relate_user_name":relate_user_name};
...
- (void)replyFinish:(BOOL)success Data:(id)result
{
    ...
    if (success) {
        ...
        model.relate_user_name = [NSString stringWithFormat:@"%@的%@",_indexItem.create_user_name,_indexItem.relation];
...
- (void)editFanilyAction:(id)sender
{
    ...
    editDetailController.indexComent = _comment ?: @"";
    editDetailController.voiceUrl = _voiceUrl;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        if (indexPath.section == [indexArr count] + 1) {
            ...
            [thanksBtn setTag:20];
            [thanksBtn setImage:CREATE_IMG(@"contact_thanks") forState:UIControlStateNormal];
...
{
    _indexItem = model;
    if ([model.create_user_type length] > 0 && [model.create_user_type isEqualToString:@"2"]) {
        //delete
        DJTGlobalManager *manager = [DJTGlobalManager shareInstance];
...
- (void)sendDeleteRequest
{
    ...
    NSDictionary *dic = @{@"school_id":manager.userInfo.schoolid,@"class_id":manager.userInfo.classid,@"reply_id":_indexItem.reply_id};
