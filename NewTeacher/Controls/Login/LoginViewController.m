...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if ([navBar respondsToSelector:@selector(setBarTintColor:)]) {
        navBar.barTintColor = CreateColor(3, 210, 126);
    }
    else
    {
        navBar.tintColor = CreateColor(3, 210, 126);
    }
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    BOOL firstLaunch = [userDef boolForKey:APP_FIRST_LAUNCH];
    if (!firstLaunch) {
        [userDef setBool:YES forKey:APP_FIRST_LAUNCH];
...
- (void)loginFinish:(BOOL)success Data:(id)result
{
    ...
    else
    {
        ...
        for (id child in ret_data) {
            ...
            [grades addObject:user];
        }
        //友盟
        [MobClick event:@"LoginId" attributes:@{@"time":[NSString stringByDate:@"yyyy-MM-dd HH:mm:ss" Date:[NSDate date]]}];
        [[DJTGlobalManager shareInstance] setGrades:grades];
        if (grades.count == 0) {
            [self.view makeToast:@"您还没有添加班级信息" duration:1.0 position:@"center"];
        }
...
- (void)openAgreement
{
    AdDetailViewController *web = [[AdDetailViewController alloc]init];
...
- (void)loginWithUsername:(NSString *)userName Password:(NSString *)password
{
    ...
    NSMutableDictionary *param = [manager requestinitParamsWith:@"teacherLogin"];
