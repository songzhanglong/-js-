...
{
    NSString *description = [NSString stringWithFormat:@"%D %@ %@ %@ %@ %@ %@ %@",_isVideo,_imgs,_uploadUrl,_param,_endUrl,_endParam,_dataSource,_account];
    return (description);
}
...
- (void)endUpload:(BOOL)suc Data:(id)result
{
    ...
    if (suc) {
        NSString *tipStr =  classCircle ? @"班级圈动态有更新" : @"班级里程有更新";
