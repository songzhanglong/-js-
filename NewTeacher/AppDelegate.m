...
- (void)configUMShare{
    ...
    [UMSocialWechatHandler setWXAppId:@"wx16e5289962aa7b66" appSecret:@"407521b4b89fb79f47dd7b015d3cfcb8" url:@"http://www.goonbaby.com"];
    //打开新浪微博的SSO开关
    //[UMSocialSinaSSOHandler openNewSinaSSOWithRedirectURL:@"http://www.goonbaby.com"];
    [UMSocialSinaSSOHandler openNewSinaSSOWithAppKey:@"18866485"
                                              secret:@"801a7bd65739d52152b767469f73e988"
                                         RedirectURL:@"http://www.goonbaby.com"];
    //设置分享到QQ空间的应用Id，和分享url 链接
    [UMSocialQQHandler setQQWithAppId:@"1104506160" appKey:@"N5xy1id2IRZ66BEo" url:@"http://www.goonbaby.com"];
    //设置支持没有客户端情况下使用SSO授权
...
- (void)popToLoginViewController
{
    ...
    [manager setGrades:nil];
    [manager setUserInfo:nil];
    [manager setStudentControls:nil];
    [manager.albumDic removeAllObjects];
    [manager setHomeCardArr:nil];
    [manager setWeekIdx:0];
    [manager setWeekList:nil];
    [manager setDeviceAttences:nil];
...
{
    DJTGlobalManager *manager = [DJTGlobalManager shareInstance];
    DJTUser *curUser = manager.grades[index];
    [manager setUserInfo:curUser];
    
    //多个孩子登录
    NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
    [userDefault setObject:manager.userInfo.schoolname ?: @"" forKey:LOGIN_SCHOOL];
...
- (void)isForcedToUpdate
{
    ...
    NSString *url = [URLFACE stringByAppendingString:@"public:version"];
    NSDictionary *dic = @{@"client":@"teacher",@"tag":@"ios"};
...
- (void)checkDevice
{
    ...
    [param setObject:manager.userInfo.schoolid forKey:@"school_id"];
        ...
        if (success) {
            ...
            if (ret_data) {
                ...
                if (have_device) {
                    ...
                    //weakManager.userInfo.hasPlayCard = YES;
...
{
    DJTGlobalManager *manager = [DJTGlobalManager shareInstance];
    if (manager.userInfo.hasPlayCard) {
        return;
    }
    
    __weak typeof(manager)weakManager = manager;
    NSString *url = [G_INTERFACE_ADDRESS stringByAppendingString:@"attence"];
    NSMutableDictionary *param = [manager requestinitParamsWith:@"getAttenceDay2"];
    [param setObject:manager.userInfo.classid forKey:@"class_id"];
    [param setObject:[NSString stringByDate:@"yyyy-MM-dd" Date:[NSDate date]] forKey:@"date"];
    NSString *text = [NSString hmacSha1:SERCET_KEY dic:param];
    [param setObject:text forKey:@"signature"];
    [DJTHttpClient asynchronousRequest:url parameters:param successBlcok:^(BOOL success, id data, NSString *msg) {
        if (success) {
            id ret_data = [data valueForKey:@"ret_data"];
            if (ret_data) {
                //0-未排班/未排课, 1-正常，2-迟到，3-早退 ，4-迟到+早退 5-缺卡(当天只刷一次卡)，6-缺勤（当天无打卡）, 7-病假，8-事假
                NSMutableArray *array = [NSMutableArray array];
                for (id attence in ret_data) {
                    NSError *error;
                    DJTDeviceAttence *attence_device = [[DJTDeviceAttence alloc] initWithDictionary:attence error:&error];
                    if (error) {
                        NSLog(@"%@",error.description);
                        continue;
                    }
                    
                    [array addObject:attence_device];
                }
                [weakManager setDeviceAttences:array];
            }
            [[NSNotificationCenter defaultCenter] postNotificationName:REFRESH_MAIN_HEADVIEW object:nil];
        }
    } failedBlock:^(NSString *description) {
...
- (void)checkPlayCard
{
    ...
    NSMutableDictionary *param = [manager requestinitParamsWith:@"checkTodayAttence"];
    [param setObject:manager.userInfo.classid forKey:@"class_id"];
    NSString *text = [NSString hmacSha1:SERCET_KEY dic:param];
    [param setObject:text forKey:@"signature"];
    [DJTHttpClient asynchronousRequest:url parameters:param successBlcok:^(BOOL success, id data, NSString *msg) {
        if (success) {
            id ret_data = [data valueForKey:@"ret_data"];
            if (ret_data) {
                NSString *have_playCard = [ret_data valueForKey:@"todayAttence"];  //0、未刷卡  1、刷卡
                if (have_playCard) {
                    weakManager.userInfo.hasPlayCard = ([have_playCard integerValue] == 1);
                }
            }
        }
    } failedBlock:^(NSString *description) {
...
- (void)dealWithError
{
    ...
    [param setObject:@"1" forKey:@"from_type"];
    NSString *text = [NSString hmacSha1:SERCET_KEY dic:param];
    [param setObject:text forKey:@"signature"];
    __weak typeof(self)weakSelf = self;
