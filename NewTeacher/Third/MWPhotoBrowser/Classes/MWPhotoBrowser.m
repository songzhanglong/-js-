...
@interface MWPhotoBrowser ()<AVAudioPlayerDelegate,UICollectionViewDelegate,UICollectionViewDataSource,DJTShareImageViewDelegate,UMSocialUIDelegate>
...
{
    UIButton *_recordBut,*_horiBut,*_numBut;
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        if (_displayNavArrows && !_displayActionButton) {
            /*
             ...
             [tmpBut setFrame:CGRectMake(0, 0, 30, 30)];
             [tmpBut setImage:[UIImage imageWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"jg1@2x" ofType:@"png"]] forState:UIControlStateNormal];
             [tmpBut addTarget:self action:@selector(popupViewControl:) forControlEvents:UIControlEventTouchUpInside];
             self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:tmpBut];
             */
        }
    }
    ...
    if (_previousButton && _nextButton && numberOfPhotos > 0) {
        ...
        if (_totalCount > 0) {
            ...
            if (_showDiggNum) {
                ...
                [label setTextColor:[UIColor whiteColor]];
                [label setBackgroundColor:[UIColor clearColor]];
                UIBarButtonItem *firstItem = [[UIBarButtonItem alloc] initWithCustomView:label];
                //[items addObject:flexSpace];
                //[items addObject:flexSpace];
...
- (void)updateNavigation {
    ...
    _downButton.enabled = enabled;_horiBut.enabled = enabled;
    _numBut.enabled = enabled;
    if (_downButton) {
        if (_delegate && [_delegate respondsToSelector:@selector(titleOfCreater:photoAtIndex:)]) {
            for (UIBarButtonItem *item in _toolbar.items) {
                if ([item.customView isKindOfClass:[UILabel class]]) {
                    UILabel *label = (UILabel *)item.customView;
                    [label setText:[_delegate titleOfCreater:self photoAtIndex:_currentPageIndex]];
                    break;
                }
            }
